...
try {
    ...
    // TASK-067-9: Legacy код месячного режима удалён (перенесён в новый модуль)
    // Если periodMode не weeks и не months, выбрасываем ошибку
    if ($periodMode !== 'weeks' && $periodMode !== 'months') {
        http_response_code(400);
        jsonResponse([
            'success' => false,
            'message' => 'Invalid periodMode. Supported values: weeks, months'
        ]);
    }
