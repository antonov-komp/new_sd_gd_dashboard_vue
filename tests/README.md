...
Эта папка содержит единую систему тестирования проекта с поддержкой различных сред выполнения, категоризации тестов и автоматизированной отчетности.

## Быстрый старт

```bash
# Запуск всех unit тестов в sandbox среде
./tests/run-tests.sh --environment sandbox --category unit

# Запуск integration тестов в локальной среде
./tests/run-tests.sh --environment local --category integration --verbose
...
### Среды тестирования
- **sandbox**: Ограниченная среда без внешних зависимостей (mock системы)
- **local**: Локальная среда с реальными API
- **ci**: Автоматизированное тестирование для CI/CD

### Категории тестов
- **unit**: Модульные тесты логики (без внешних зависимостей)
- **integration**: Тесты взаимодействия компонентов
- **e2e**: Полные сценарии с API

## Файлы проекта

### Устаревшие файлы (корневая директория tests/)

Следующие файлы остались для совместимости, но рекомендуется использовать новую категоризированную структуру:
