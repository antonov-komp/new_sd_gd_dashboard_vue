...
## Критерии приёмки
- [x] Код месячного режима удалён из legacy
- [x] Legacy файл стал тонким прокси
- [x] Синтаксис PHP корректен
- [x] Прокси работает корректно:
  - weeks режим перенаправляется на новый модуль
  - months режим перенаправляется на новый модуль
- [x] Обработка ошибок работает
- [x] Файл стал значительно короче (удалено ~1022 строки legacy кода)
