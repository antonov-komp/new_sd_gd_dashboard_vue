...
- [x] Сервис `admissionClosureService.js` поддерживает `includeCarryoverTickets`
...
- [x] Цвета обновлены: Новые (синий), Закрытые (зелёный), Переходящие (оранжевый)
- [x] Тултипы показывают значения для всех трёх категорий
- [x] При клике на «Переходящие» эмитится событие `open-carryover`
- [x] Легенды обновлены для всех трёх категорий
- [x] Визуальное оформление соответствует существующему стилю
...
- 2025-12-16 18:00 (UTC+3, Брест): Реализовано обновление графиков для отображения трёх категорий
  - Обновлён сервис `admissionClosureService.js` для поддержки `includeCarryoverTickets`
  - Добавлен третий dataset для переходящих тикетов в линейный и столбчатый графики
  - Добавлен третий сектор в круговую диаграмму
  - Добавлена summary-карточка «Переходящие» с обработчиком клика
  - Обновлены цвета: добавлен `chartColors.carryover = '#ff9800'`
  - Обновлён обработчик кликов для поддержки переходящих тикетов
  - Добавлено событие `open-carryover` в компонент графика
  - Обновлён дашборд для обработки события `open-carryover` (попап будет реализован в TASK-044-05)
