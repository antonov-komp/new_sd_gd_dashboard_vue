...
- `TicketCard.vue` переведён на новые поля приоритета (`priorityId`, `priorityLabel`, `priorityColors`), legacy `priority` сохранён.
- Чип и левый бордер карточки теперь берут цвета из конфига; добавлен нейтральный fallback («Не указано»).
- Убраны жёсткие классы `priority-high/medium/low`; стилизация через inline-стили/бордер, сохранены DnD и клики.
- JSDoc обновлён с упоминанием UF_CRM_7_UF_PRIORITY и конфига приоритетов.
