...
- [ ] Реализована функция `isCarryoverTicket()` с проверкой всех условий
- [ ] Переходящие тикеты фильтруются корректно:
  - [ ] `createdTime < weekStartUtc` (создан до начала недели)
  - [ ] `stageId` ∈ рабочих стадий (DT140_12:UC_0VHWE2, DT140_12:PREPARATION, DT140_12:CLIENT)
  - [ ] `stageId` ∉ закрывающих стадий
- [ ] Фильтр по продукту `product=1C` применяется первым шагом
- [ ] Подсчитывается количество переходящих тикетов (`$carryoverCount`)
- [ ] Тикеты, созданные в текущую неделю, не попадают в переходящие
- [ ] Закрытые тикеты не попадают в переходящие
- [ ] Логика работает корректно с существующим кодом (не ломает `newCount` и `closedCount`)
