...
- **2025-12-06 19:20 (UTC+3, Брест):** Выполнен рефакторинг:
  - Импортированы утилиты из `ticket-utils.js` (KEEPER_OBJECTS_ID, getAssignedById, parseEmployeeId, isZeroPointTicket)
  - Удалена константа KEEPER_OBJECTS_ID из ticket-grouper.js (теперь импортируется из утилит)
  - Обновлена функция `getZeroPointTickets()` - использует `isZeroPointTicket()`
  - Обновлена функция `groupTicketsByStages()` - использует `getAssignedById()` и `parseEmployeeId()`
  - Обновлена функция `extractUniqueEmployeeIds()` - использует утилиты вместо сложной логики
