...
- **2025-12-17 14:22 (UTC+3, Брест):** Задача завершена
  - Создан компонент `PeriodModeSelector.vue` с переключателем режимов
  - Интегрирован в `GraphAdmissionClosureDashboard.vue`
  - Обновлена логика `loadData()` для работы с `periodMode`
  - Убран `weekPickerMode` из `FiltersPanel` (установлен в `false`)
  - Реализовано сохранение выбора в localStorage
  - Добавлена синхронизация между вкладками браузера
  - Режим "4 недели" работает как раньше (обратная совместимость)
  - Режим "3 месяца" показывает сообщение о разработке
