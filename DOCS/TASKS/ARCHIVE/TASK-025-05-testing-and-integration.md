...
- `dashboard-sector-1c-service.js` обновлён: маппит `UF_CRM_7_UF_PRIORITY` через `priority-config`, возвращает `priorityId/priorityLabel/priorityColors/priorityBitrixValue` + legacy `priority`; обратный маппинг использует текстовое значение, сохраняет поддержку старых кодов.
- `ticket-details-service.js` обновлён: детали тикета теперь содержат `priorityId/priorityLabel/priorityColors/priorityBitrixValue` с fallback, legacy `priority` оставлен.
- Fallback и dev-логирование неизвестных значений наследуются из `priority-config`, прод-режим остаётся тихим.
- Остальные компоненты/фильтры/экспорты явных ссылок на старые классы/поля не выявлено (поиск по репо), но ручная регрессия UI/экспортов всё ещё требуется.
