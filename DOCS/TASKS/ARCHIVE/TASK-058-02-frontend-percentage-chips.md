...
### Ошибка 1: Проценты не отображаются

**Причина:** `previousPeriodData` отсутствует или содержит null значения

**Решение:**
- Проверить, что Backend возвращает `previousPeriodData` (TASK-058-01 должен быть выполнен)
- Добавить проверку в computed-свойство:
  ```javascript
  const newTicketsPercentage = computed(() => {
    if (!props.data?.previousPeriodData) {
      console.warn('previousPeriodData is missing');
      return null;
    }
    // ... остальной код
  });
...
### Ошибка 2: Проценты показывают NaN или Infinity

**Причина:** Деление на ноль или невалидные данные

**Решение:**
- Добавить проверку на `previous === 0` перед расчетом
- Добавить проверку на `isFinite()` после расчета
- Добавить валидацию типов данных

### Ошибка 3: Проценты отображаются, но неправильно

**Причина:** Неправильная формула расчета или неверные данные

**Решение:**
- Проверить формулу: `((current - previous) / previous) * 100`
- Проверить, что `current` и `previous` — числа, а не строки
- Добавить логирование для отладки:
  ```javascript
  console.log('Current:', current, 'Previous:', previous, 'Percentage:', percentage);
  ```

### Ошибка 4: Стили не применяются

**Причина:** Неправильные классы или конфликт стилей

**Решение:**
- Проверить, что классы `.positive` и `.negative` определены
- Проверить, что используется `scoped` в `<style scoped>`
- Проверить специфичность CSS селекторов
...
- Использовать существующую функцию для форматирования абсолютных значений
- Не дублировать логику форматирования

**Computed-свойства:**
- Использовать существующие computed-свойства (`totalNewTickets`, `formattedTotalNewTickets`)
- Добавить новые computed-свойства для процентов рядом с существующими
