...
1. **Левый верхний угол** — отображение отдела заказчика (`UF_CRM_7_DEPARTMENT_HEAD`) с ограничением 10 символов
2. **Левый нижний угол** — отображение даты создания (`createdTime`) с визуальными акцентами в зависимости от давности
...
- Ограничить длину до 10 символов (если больше — обрезать с `...`)
...
- [ ] Значение нормализуется (trim, ограничение 10 символов)
...
### Этап 3: Добавление отображения отдела заказчика в левом верхнем углу

**Цель:** Добавить элемент отображения отдела заказчика в левый верхний угол карточки тикета.
...
- [ ] Элемент отдела заказчика отображается в левом верхнем углу
- [ ] Текст ограничен 10 символами (с обрезкой и многоточием)
...
**Цель:** Добавить элемент отображения даты создания в левый нижний угол карточки с визуальными акцентами.
...
- [ ] Визуальные акценты работают для всех категорий давности
...
- Ограничение длины отдела заказчика: 10 символов
- Визуальные акценты для 8 категорий давности

## Критерии приёмки

- [ ] Поле `UF_CRM_7_DEPARTMENT_HEAD` извлекается из Bitrix24 и добавляется в маппер
- [ ] Отдел заказчика отображается в левом верхнем углу карточки (ограничение 10 символов)
- [ ] Дата создания отображается в левом нижнем углу карточки (формат DD.MM.YYYY)
- [ ] Визуальные акценты применяются в зависимости от давности (8 категорий)
- [ ] Все категории давности имеют соответствующие цвета и стили
- [ ] Граничные случаи обработаны (отсутствие данных, некорректные даты)
- [ ] Код соответствует стандартам проекта (ESLint, комментарии)
- [ ] JSDoc комментарии обновлены во всех файлах
