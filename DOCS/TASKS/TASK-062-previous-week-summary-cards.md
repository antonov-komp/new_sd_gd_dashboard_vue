...
- **2025-12-23 17:30 (UTC+3, Брест):** Задача реализована (базовая версия)
  - Создано computed-свойство `previousWeekData` для данных предыдущей недели
  - Создано computed-свойство `currentWeekMeta` для метаданных текущей недели
  - Добавлена функция `formatWeekDates` для форматирования дат недели
  - Обновлён template: существующие карточки обёрнуты в блок текущей недели
  - Добавлен блок предыдущей недели с 4 карточками (новые, закрытые, переходящие, стадии)
  - Добавлен визуальный разделитель между блоками
  - Реализованы стили для визуального разделения (текущая неделя выделена, предыдущая приглушена)
  - Добавлены адаптивные стили для планшетов и мобильных устройств
  - Карточки предыдущей недели не кликабельны (только для отображения)
  - Для стадий предыдущей недели используются общие стадии (как рекомендовано)

- **2025-12-23 18:00 (UTC+3, Брест):** Расширение функциональности — кликабельность и проценты
  - **Кликабельность карточек предыдущей недели:**
    - Карточки предыдущей недели сделаны кликабельными
    - Добавлен обработчик `handlePreviousWeekSummaryClick` для карточек предыдущей недели
    - Создано computed-свойство `previousWeekMeta` для метаданных предыдущей недели
    - Модифицированы события `open-stages`, `open-responsible`, `open-carryover` для передачи метаданных недели
    - Обновлён родительский компонент `GraphAdmissionClosureDashboard.vue`:
      - Добавлены обработчики `handleOpenStages`, `handleOpenResponsible`, `handleOpenCarryover`
      - Добавлено состояние `selectedWeekMeta` для хранения метаданных выбранной недели
      - Модальные окна получают метаданные из `selectedWeekMeta` (при наличии) или из `chartMeta` (fallback)
    - Обновлены стили: кликабельные карточки имеют `cursor: pointer` и изменение `border-color` при наведении
  
  - **Проценты изменения:**
    - Добавлена функция `calculatePercentage()` для расчета процента изменения между двумя значениями
    - Добавлена функция `formatPercentage()` для форматирования процента для отображения
    - Создано computed-свойство `prePreviousWeekData` для данных предпредыдущей недели
    - Создано computed-свойство `currentWeekPercentages` для процентов текущей недели (относительно предыдущей)
    - Создано computed-свойство `previousWeekPercentages` для процентов предыдущей недели (относительно предпредыдущей)
    - Добавлены визуальные индикаторы процентов:
      - Стрелки ↑ (рост) и ↓ (снижение)
      - Цвета: зеленый для положительных процентов, красный для отрицательных
      - Метка с номером недели: "к неделе X" для четкого указания, к какой неделе относится процент
      - Tooltip с полной информацией о проценте изменения
    - Проценты отображаются для всех 3 карточек (новые, закрытые, переходящие) в обоих блоках:
      - Текущая неделя: проценты относительно предыдущей недели
      - Предыдущая неделя: проценты относительно предпредыдущей недели
    - Добавлены стили для визуальных индикаторов:
      - `.percentage-indicator` — базовые стили
      - `.percentage-indicator--positive` — зеленый цвет для роста
      - `.percentage-indicator--negative` — красный цвет для снижения
      - Адаптивность для мобильных устройств

- **2025-12-23 19:00 (UTC+3, Брест):** Расширение функциональности — сравнение значений недель
  - **Сравнение значений в карточках предыдущей недели:**
    - В карточках предыдущей недели добавлено отображение двух значений одновременно
    - Слева: значение предыдущей недели (51) — основной цвет, размер 24px
    - Справа: значение предпредыдущей недели (50) — синий цвет (#007bff), размер 24px
    - Оба значения выровнены в одну горизонтальную линию (baseline alignment)
    - Добавлены метки с номерами недель под каждым значением
    - Создан контейнер `.summary-card__values-comparison` для размещения двух значений
    - Добавлены стили для унификации отображения:
      - `.summary-card__value--current-week` — для простых карточек
      - `.summary-card__value-main--current-week` — для карточек с разбивкой
      - Оба класса имеют одинаковые стили: размер 24px, жирность 700, синий цвет
    - Значения предпредыдущей недели отображаются для всех 3 карточек (новые, закрытые, переходящие)
    - Адаптивность: на мобильных устройствах значения отображаются вертикально
...
1. **Блок summary-карточек для предыдущей недели:**
   - ✅ Создан блок с 4 карточками для предыдущей недели
   - ✅ Визуальное разделение между блоками (разделительная линия, разные фоны)
   - ✅ Заголовки блоков с номерами недель и датами
   - ✅ Адаптивность для всех устройств

2. **Кликабельность карточек:**
   - ✅ Карточки предыдущей недели кликабельны
   - ✅ При клике открываются попапы с данными предыдущей недели
   - ✅ Передача метаданных недели в события и модальные окна
   - ✅ Визуальная индикация кликабельности (курсор, hover-эффекты)

3. **Проценты изменения:**
   - ✅ Проценты для текущей недели (относительно предыдущей)
   - ✅ Проценты для предыдущей недели (относительно предпредыдущей)
   - ✅ Визуальные индикаторы: стрелки ↑↓, цвета (зеленый/красный)
   - ✅ Четкое указание номера недели, к которой относится процент
   - ✅ Обработка граничных случаев (деление на ноль, null, NaN)

4. **Сравнение значений недель:**
   - ✅ В карточках предыдущей недели отображаются два значения одновременно
   - ✅ Слева: значение предыдущей недели (51) — основной цвет
   - ✅ Справа: значение предпредыдущей недели (50) — синий цвет для визуального отличия
   - ✅ Оба значения одинакового размера (24px) и стиля (жирность 700)
   - ✅ Значения выровнены в одну горизонтальную линию (baseline alignment)
   - ✅ Метки с номерами недель под каждым значением
   - ✅ Унифицированные стили для всех типов карточек (с разбивкой и без)
   - ✅ Адаптивность: вертикальное расположение на мобильных устройствах

### Технические детали

**Изменённые файлы:**
- `vue-app/src/components/graph-admission-closure/GraphAdmissionClosureChart.vue`
  - Добавлены computed-свойства: `previousWeekData`, `currentWeekMeta`, `previousWeekMeta`, `prePreviousWeekData`
  - Добавлены computed-свойства для процентов: `currentWeekPercentages`, `previousWeekPercentages`
  - Добавлены функции: `formatWeekDates()`, `calculatePercentage()`, `formatPercentage()`
  - Добавлены обработчики: `handlePreviousWeekSummaryClick()`
  - Обновлён template: 
    - Блоки недель с визуальным разделением
    - Индикаторы процентов с визуальными стрелками и цветами
    - Контейнер сравнения значений `.summary-card__values-comparison`
    - Отображение двух значений (предыдущая и предпредыдущая неделя) в карточках предыдущей недели
  - Добавлены стили: 
    - Визуальное разделение блоков (разделительная линия, разные фоны)
    - Индикаторы процентов (зеленый/красный, стрелки)
    - Сравнение значений (выравнивание по baseline, унифицированные стили)
    - Адаптивность для всех устройств

- `vue-app/src/components/graph-admission-closure/GraphAdmissionClosureDashboard.vue`
  - Добавлено состояние `selectedWeekMeta` для хранения метаданных выбранной недели
  - Добавлены обработчики: `handleOpenStages()`, `handleOpenResponsible()`, `handleOpenCarryover()`
  - Обновлены модальные окна для использования `selectedWeekMeta`

**Размеры production build (финальные):**
- `GraphAdmissionClosureDashboard-D-KZEE2R.js`: 130.42 kB (33.60 kB gzip)
- `GraphAdmissionClosureDashboard-Bb9QPVR-.css`: 65.37 kB (8.12 kB gzip)
...
- ✅ Все 4 карточки отображаются для предыдущей недели
- ✅ Визуальное разделение между блоками (разделительная линия, разные фоны)
- ✅ Карточки предыдущей недели кликабельны и открывают попапы
- ✅ Проценты изменения отображаются для всех карточек
- ✅ Визуальные индикаторы (стрелки, цвета) работают корректно
- ✅ Четкое указание номера недели, к которой относится процент
- ✅ В карточках предыдущей недели отображаются два значения (предыдущая и предпредыдущая неделя)
- ✅ Значения выровнены в одну горизонтальную линию
- ✅ Значения предпредыдущей недели имеют одинаковый размер и стиль во всех карточках
- ✅ Адаптивность на всех устройствах
- ✅ Обработка граничных случаев (меньше 2 недель данных, деление на ноль)

### Финальный результат

**Визуальная структура:**
- **Текущая неделя (52):** Показывает значение и процент изменения относительно предыдущей недели (51)
- **Предыдущая неделя (51):** 
  - Показывает значение недели 51 (слева) и значение недели 50 (справа, синий цвет)
  - Показывает процент изменения недели 51 относительно недели 50
  - Оба значения одинакового размера и стиля, выровнены в одну линию

**Технические достижения:**
- Полная поддержка сравнения трех недель одновременно (текущая, предыдущая, предпредыдущая)
- Унифицированные стили для всех типов карточек
- Корректная обработка всех граничных случаев
- Оптимизированная производительность (computed-свойства)
