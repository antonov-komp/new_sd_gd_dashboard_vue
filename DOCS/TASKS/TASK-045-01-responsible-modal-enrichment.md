# TASK-045-01: Обогащение данных тикетов в ResponsibleModal.vue
...
Заменить ручное маппирование тикетов в `ResponsibleModal.vue` на использование функции `prepareTicketsForDisplay()` для полного обогащения данных тикетов.

## Контекст

**Текущее состояние:**
- В `ResponsibleModal.vue` (строки 263-281) тикеты подготавливаются вручную с минимальными полями
...
- `tickets` — массив тикетов из API (`employeeTickets`)
...
1. Открыть попап `ResponsibleModal` (клик на «Закрытые» на графике)
2. Выбрать сотрудника с тикетами
...
- 2025-12-16 20:00 (UTC+3, Брест): Создана подзадача для обогащения данных тикетов в ResponsibleModal.vue
