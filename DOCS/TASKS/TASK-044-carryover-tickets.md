...
- [ ] API возвращает `carryoverTickets` при `includeCarryoverTickets: true`
- [ ] Переходящие тикеты фильтруются корректно:
  - [ ] `createdTime < weekStartUtc` (создан до начала недели)
  - [ ] `stageId` ∈ рабочих стадий (DT140_12:UC_0VHWE2, DT140_12:PREPARATION, DT140_12:CLIENT)
  - [ ] `stageId` ∉ закрывающих стадий
...
- [ ] При клике на «Переходящие» открывается попап `CarryoverDurationModal`
- [ ] В попапе отображается градация «По срокам» с количеством тикетов в каждой категории:
  - [ ] Менее 1 месяца
  - [ ] Более 1 месяца
  - [ ] Более 2 месяцев
  - [ ] Более полугода
  - [ ] Более года
- [ ] При клике на категорию срока открывается уровень 2 со списком тикетов
- [ ] Тикеты отображаются через компонент `TicketCard`
- [ ] Кнопка «Назад» возвращает на уровень 1
- [ ] Цвета и легенды обновлены для трёх категорий
- [ ] Тултипы показывают значения для всех трёх категорий
- [ ] API возвращает `carryoverTicketsByDuration[]` при `includeCarryoverTicketsByDuration: true`
