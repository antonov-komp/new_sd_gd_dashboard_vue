...
- **2026-01-12 14:30 (UTC+3, Брест)**: Решена проблема с отсутствующими стилями. CSS code splitting был отключен (`cssCodeSplit: false`), так как разделенные CSS файлы не подключались к HTML. Теперь все стили объединены в один файл style-Cr0ThMfa.css (233KB), который корректно загружается