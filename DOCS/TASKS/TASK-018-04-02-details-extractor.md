...
- [ ] Класс `EventDetailsExtractor` создан и реализован
- [ ] Метод `extract()` реализован
- [ ] Метод `extractTaskEvent()` реализован
- [ ] Метод `extractTaskCommentEvent()` реализован
- [ ] Метод `extractSmartProcessEvent()` реализован
- [ ] Статический метод `extractEventDetails()` реализован для обратной совместимости
- [ ] Поддержка всех типов событий добавлена
- [ ] Валидация извлечённых данных реализована
...
- [ ] **Детали событий корректно отображаются в интерфейсе Vue.js**
- [ ] **Поля деталей поддерживают фильтрацию в `WebhookLogFilters.vue` (например, `task_id`, `entity_id`)**
