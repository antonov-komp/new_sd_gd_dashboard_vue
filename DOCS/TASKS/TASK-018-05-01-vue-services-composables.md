...
- [ ] Файл `vue-app/src/types/webhook-logs.js` создан с типизированными интерфейсами
- [ ] Валидаторы `isValidWebhookLogEntry` и `isValidEventDetails` реализованы
- [ ] Нормализация `normalizeWebhookLogEntry` и `normalizeWebhookLogEntries` реализована
- [ ] `WebhookLogsApiService` обновлён для работы с новым API
- [ ] Валидация данных в `WebhookLogsApiService` добавлена
- [ ] Нормализация записей в `WebhookLogsApiService` реализована
- [ ] Обработка ошибок в `WebhookLogsApiService` улучшена
- [ ] `useRealtime` composable обновлён для работы с новым SSE endpoint
- [ ] Валидация новых логов в `useRealtime` добавлена
- [ ] Файл `vue-app/src/utils/webhook-validators.js` создан
- [ ] Файл `vue-app/src/utils/webhook-formatters.js` создан
...
- [ ] **Совместимость с Vue.js компонентами сохранена (WebhookLogsPage, WebhookLogList, WebhookLogDetails)**
- [ ] **Формат данных соответствует ожиданиям Vue.js компонентов**
