...
- [x] Бэкенд-источник унифицирован с модулями 1/2: `api/graph-1c-admission-closure.php` теперь грузит тикеты смарт-процесса 140 по тем же целевым стадиям (formed/review/execution) + закрывающие стадии, фильтр product=1C первым шагом (нормализация как в `sector-filter.js`), счёт new/closed по createdTime и movedTime/updatedTime.  
