...
## Прогресс (обновлено 2025-12-15 18:00 (UTC+03:00, Brest))

### ✅ Этап 1 — Подготовка и анализ
- Изучены текущие графики/доступы/фильтры (`GraphStateDashboard.vue`, `GraphStateChart.vue`)
- Подтверждён первичный фильтр по `UF_CRM_7_TYPE_PRODUCT` через `sector-filter.js`
- Логика ответственных/нулевой точки подтверждена в `ticket-utils.js`
- Принято решение унифицировать источник данных с модулями 1/2 (та же цепочка получения тикетов)

### ✅ Этап 2 — Бэкенд-контракт и данные
- Зафиксирован REST-контракт в `TASK-041-02-backend-contract-and-data.md`
- Endpoint: `/api/graph-1c-admission-closure.php`
- Входные параметры: `product + weekStartUtc/weekEndUtc`
- Формат ответа: UTC-ответ `meta/data/series/stages/responsible`
- Нормализация ответственных и списка закрывающих стадий
- **Исправлена синтаксическая ошибка PHP** (дублирующийся код в цикле)
- **Оптимизирована логика запросов**: два запроса к Bitrix24 (созданные в неделю + закрытые в неделю) вместо полной выборки всех тикетов

### ✅ Этап 3 — UI без селектора этапов
- Создан новый дашборд `GraphAdmissionClosureDashboard.vue`
- Интегрирован `FiltersPanel` с параметром `hideStages=true`
- Селектор этапов скрыт
- **Изменён layout**: фильтры по центру на всю ширину, график под фильтрами (вертикальная компоновка)

### ✅ Этап 4 — Графики (агрегация неделя)
- Реализован `GraphAdmissionClosureChart.vue`
- Линейный график: недельные агрегаты new/closed
- Столбчатый график: недельные агрегаты new/closed
- Круговая диаграмма: сравнение "Новые" vs "Закрытые"
- Стадж-теги и легенды адаптированы

### ✅ Этап 5 — Попап 1-го уровня (изолированный)
- Создан `ResponsibleModal.vue` — отдельный изолированный компонент
- Открывается по клику на график/кнопку "Ответственные"
- Отображает агрегат ответственных с количеством тикетов
- Обработка пустых данных

### ✅ Этап 6 — Интеграция и навигация
- Добавлен маршрут `/dashboard/graph-admission-closure` в `router/index.js`
- Добавлена кнопка в `reports-config.js`
- Подключена проверка доступа через навигацию
- **Создан компонент выбора недели `WeekPicker.vue`**:
  - Выпадающий список с барабаном прокрутки
  - Автоматическая генерация списка недель (52 недели назад)
  - Выбор недели только при нажатии "Применить" (пересчёт данных)
  - Кнопки "Применить" и "Отмена"
  - Закрытие при клике вне компонента
- **Интегрирован WeekPicker в FiltersPanel**:
  - Добавлен prop `weekPickerMode` для включения режима выбора недели
  - Добавлены props `selectedWeek` и `weeksCount`
  - Обновлён `GraphAdmissionClosureDashboard` для использования WeekPicker

### ✅ Этап 7 — Тестирование
- Проверена работа API endpoint (исправлена 404 ошибка)
- Проверено отображение данных за выбранную неделю
- Проверена работа фильтров и выбор недели
- Проверена работа попапа ответственных
- Модуль полностью функционален и готов к использованию

## Итоговые изменения

### Созданные файлы:
1. `vue-app/src/components/graph-admission-closure/GraphAdmissionClosureDashboard.vue` — основной дашборд
2. `vue-app/src/components/graph-admission-closure/GraphAdmissionClosureChart.vue` — компонент графиков
3. `vue-app/src/components/graph-admission-closure/ResponsibleModal.vue` — попап ответственных
4. `vue-app/src/components/filters/WeekPicker.vue` — компонент выбора недели с барабаном
5. `vue-app/src/services/graph-admission-closure/admissionClosureService.js` — сервис для работы с API
6. `api/graph-1c-admission-closure.php` — бэкенд endpoint

### Обновлённые файлы:
1. `vue-app/src/components/filters/FiltersPanel.vue` — добавлена поддержка WeekPicker
2. `vue-app/src/router/index.js` — добавлен маршрут для нового модуля
3. `vue-app/src/config/reports-config.js` — добавлена кнопка в меню отчётов

### Ключевые решения:
- **Унификация источника данных**: бэкенд использует ту же логику выборки тикетов, что и модули 1/2 (смарт-процесс 140, целевые стадии, фильтр product=1C первым шагом)
- **Оптимизация запросов**: два целевых запроса к Bitrix24 вместо полной выборки всех тикетов
- **Выбор недели**: реализован через выпадающий список с барабаном прокрутки, пересчёт данных только при нажатии "Применить"
- **Layout**: фильтры и график расположены вертикально, оба на всю ширину
