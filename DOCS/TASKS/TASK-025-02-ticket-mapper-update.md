...
- `ticket-mapper.js` читает `UF_CRM_7_UF_PRIORITY` во всех вариантах регистра/кейса и нормализует значение.
- Маппинг приоритета теперь через `priority-config`: отдаём `priorityId`, `priorityLabel`, `priorityColors`, `priorityBitrixValue`, оставлен legacy `priority`.
- `mapPriorityToBitrix` возвращает текстовое значение из конфига, поддерживает старые числовые коды как fallback.
- Dev-логирование неизвестных значений реализовано через конфиг, рантайм не падает.
